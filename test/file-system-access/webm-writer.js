 ...
 *
 * You can supply a FileSystemWritableFileStream WritableStreamDefaultWriter, in which case the BlobBuffer is just used as temporary storage before it writes it
 * through to the disk.
 *
 * By Nicholas Sherlock
 *
...
(function () {
  var BlobBuffer = function (fs) {
    return function (destination) {
       ...
       *
       * If a FileSystemWritableFileStream WritableStreamDefaultWriter was not provided, the promise is resolved with a Blob that represents the completed BlobBuffer
       * contents. You can optionally pass in a mimeType to be used for this blob.
       *
       * If a FileSystemWritableFileStream WritableStreamDefaultWriter was provided, the promise is resolved with null as the first argument.
       */
 ...
 * it can stream Blobs directly to a FileSystemWritableFileStream WritableStreamDefaultWriter without buffering the entire video in memory.
 *
 * When FileSystemWritableFileStream WritableStreamDefaultWriter is not available or not desired, it can buffer the video in memory as a series of Blobs which are
    ...
    return function (options) {
      var MAX_CLUSTER_DURATION_MSEC = 5000,
        ...
        optionDefaults = {
          ...
          fileWriter: null, // FileSystemWritableFileStream WritableStreamDefaultWriter in order to stream to a file instead of buffering to memory (optional)
      ...
      this.addFrame = function (canvas, frameDuration, width, height) {
        ...
        var webP = renderAsWebP(canvas, options.quality);
